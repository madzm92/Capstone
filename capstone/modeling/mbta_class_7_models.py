...
def plot_residuals(y_test, y_pred, model_name="Model", save_dir="residual_plots"):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"{model_name}_residuals_vs_predicted.png"))
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"{model_name}_residual_distribution.png"))
    plt.close()
...
traffic = gpd.read_postgis(
    ...
    where functional_class in ('(5) Major Collector', '(6) Minor Collector', '(7) Local Road or Street')
    """,
    engine,
    geom_col="geom"
)
